...
bash scripts/run_benchmark.sh baseline    # 3 backend x 5 seed -> logs/
bash scripts/run_visualize.sh baseline    # 그래프·표 -> results/
```
...
```bash
bash scripts/run_benchmark.sh tuning-ablation
bash scripts/run_visualize.sh tuning-ablation
```
...
```bash
bash scripts/run_benchmark.sh backend-comparison
bash scripts/run_visualize.sh backend-comparison
```
