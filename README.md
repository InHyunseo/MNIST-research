...
|---|---|---|
| [`denoising-multitask`](denoising-multitask/) | n-MNIST 분류에서 clean 복원 보조 과제가 LeNet 성능에 미치는 영향 | 30-seed 실험 완료 |
...
AWGN, motion blur, reduced contrast + AWGN이 적용된 n-MNIST에서 classification-only
LeNet과 clean image 복원 decoder를 함께 학습한 모델을 30개의 paired seed로 비교한다.
Decoder는 학습 중에만 보조 과제로 사용하고 추론에서는 실행하지 않는다.

- AWGN: `+0.002 pp`, paired t-test `p=0.9305`
- Motion blur: `+0.062 pp`, 95% CI `[+0.024, +0.101]`, `p=0.0026`
- Reduced contrast + AWGN: `+0.025 pp`, `p=0.5116`

복원 보조 과제의 유의한 분류 성능 향상은 motion blur에서만 확인됐다. 실행 방법,
모델 구조, 전체 결과와 figure 목록은
[`denoising-multitask/README.md`](denoising-multitask/README.md)에 정리돼 있다.
...
```text
MNIST-research/
├── denoising-multitask/     Noisy classification + clean reconstruction
...
각 프로젝트는 코드를 공유하는 단일 Python package가 아니다. Config, 가상환경,
...
```bash
# Noisy classification + reconstruction
cd denoising-multitask
