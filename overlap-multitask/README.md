...
Multitask 모델은 LeNet의 `[16,16,16]` convolution feature를 분류 head와 복원 decoder가
공유한다. Decoder는 feature를 전역 projection한 뒤 두 장의 `28×28` 원본 숫자를 출력한다.

```text
LeNet feature [16×16×16]
  → Flatten → FC(4096→256) → FC(256→16×7×7)
  → ConvTranspose(16→8, k4, s2, p1)
  → ConvTranspose(8→2, k4, s2, p1) → Sigmoid
  → Reconstructed sources 2×28×28
```

두 출력의 순서가 없으므로 direct/swapped assignment의 intensity-balanced L1을 sample별로
비교하는 PIT loss를 사용한다. 전체 loss는 classification BCE와 reconstruction loss의
가중합이다. Seed 0에서 `λ ∈ {0.05, 0.1, 0.2}`를 validation exact-match로 선택한 뒤,
선택된 λ로 seeds 0–9를 처음부터 공동학습한다. 모든 LeNet parameter는 학습되며 decoder는
추론 시 제거할 수 있다.
...
pair별 accuracy delta와 PIT-matched L1·MSE·PSNR을 `results/multitask/`에 저장한다.
...
│   └── multitask/      # pilot, 선택 결과, final checkpoint와 이력
└── results/
    ├── baseline/       # baseline metrics와 PNG
    └── multitask/      # paired 비교 metrics와 PNG
```
...
합성한 조건에 한정된다. 복원은 입력에서 소실된 정보를 완벽히 되찾는 목표가 아니라 분류
encoder의 보조 supervision이다. 실제 객체 가림 문제, 다른 데이터셋 또는 다른 모델 구조로의
직접 일반화는 이 실험의 주장 범위가 아니다.
