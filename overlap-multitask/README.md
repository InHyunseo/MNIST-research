...
합성한 조건에 한정된다. 복원 결과는 입력 좌표의 10개 class-specific map이며, 실제
두 class의 map만 원본 숫자를 target으로 갖는다. Maximum 합성에서 소실된 pixel을 완벽히
되찾는다고 주장하지 않으며 분류 encoder의 보조 supervision으로 해석한다. 같은 class의
