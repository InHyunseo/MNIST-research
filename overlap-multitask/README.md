...
두 `28×28` 숫자를 `76×76` 흑백 canvas에 배치하고 두 canvas의 pixel-wise
산술평균으로 합성한다. 정답은 두 숫자 class 위치가 1인 10차원 multi-hot
vector다. 모델은 class별
...
LeNet encoder
  ...
  bottleneck [16×16×16]             │             │
    ...
    → DoubleConv(12→6) → Conv(6→10, k1)
    → Class-specific logits [10×64×64]
```

복원 target은 `[10,64,64]` semantic map이다. 숫자 `k`의 원본은 입력 canvas와 같은
좌표로 class channel `k`에 배치하며 나머지 channel은 0이다. 서로 다른 class의 숫자만
사용하므로 channel 의미가 고정되고 PIT가 필요 없다. 겹친 pixel은 두 class에 동시에
속할 수 있어 channel별 sigmoid를 사용한다.

Reconstruction loss는 foreground/background를 각각 정규화한 weighted pixel BCE와,
실제 숫자가 있는 두 channel의 soft Dice loss를 1:1로 합친다. 검은 배경의 개수가 loss를
지배하지 않고, 다른 숫자의 획을 함께 출력하면 Dice가 낮아진다.
...
pair별 accuracy delta를 계산한다. 복원은 spatial map의 foreground Dice와 balanced BCE를
...
│   └── multitask/      # semantic U-Net·loss·pilot·비교 평가·그림·main
...
pixel-wise mean으로 합성한 조건에 한정된다. 복원 결과는 입력 좌표의 10개
class-specific map이며, 실제 두 class의 map만 원본 숫자를 target으로 갖는다.
Mean 합성은 두 source의 강도 정보를 합성 입력에 남기지만, 하나의 pixel에서 각 source의
