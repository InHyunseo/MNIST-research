...
### Reconstruction recognizability

복원 성능의 발표용 대표 지표는 clean MNIST만 학습한 별도 LeNet이 복원된 `28×28`
숫자를 맞힌 비율이다. 이 평가기는 multitask encoder와 parameter를 공유하지 않고,
학습 후 완전히 고정하므로 복원 모델의 학습에는 영향을 주지 않는다.

```text
Reconstructed source [2×28×28]
  → frozen clean-MNIST LeNet
  → class prediction [2]
  → source label과 비교
```

- `clean_mnist_test_accuracy`: 평가용 LeNet이 깨끗한 MNIST 공식 test 이미지를 맞힌
  비율이며, 네 모델의 발표 점수에는 포함하지 않는 평가기 검증값이다.
- `reconstruction_accuracy`: 복원된 두 숫자를 평가용 LeNet으로 판독한 뒤, 출력 순서와
  무관하게 두 class를 모두 맞힌 입력의 비율이다.

복원 branch 자체의 가독성을 분류 성능과 분리해 측정하기 위해 정답 source class를
조건으로 생성한 결과에 이 지표를 적용한다. 실제 label이 없는 입력에서는 joint 모델이
먼저 예측한 Top-2 class를 decoder 조건으로 사용해 label과 두 복원을 함께 출력한다.
...
pair별 accuracy delta를 계산한다. 복원 성능은 frozen MNIST 평가기가 복원된 두 숫자를
모두 맞힌 입력의 비율 하나로 평가한다. 최종 비교에서는 reconstruction-only와 multitask
reconstruction에 같은 판정 함수를 적용한다. 결과는 `results/multitask_compact/`에
저장한다. 학습 곡선의 accuracy와 loss y축은 `0–1`로 고정한다.
...
.
├── README.md
├── main.py                 # MNIST 평가기·baseline·multitask 통합 CLI
├── requirements.txt
├── configs/
│   ├── mnist_overlap.yaml
...
│   ├── mnist_classifier.py # 독립 복원 가독성 평가기
...
가장 바깥 `main.py`가 권장 통합 진입점이다. 인자 없이 실행하면 독립 MNIST 평가기,
baseline, multitask를 차례로 완료하거나 호환되는 checkpoint를 재사용한다.
...
```bash
python main.py --model mnist --device cpu
python main.py --model baseline --device cpu
python main.py --model multitask --device cpu
```

기존 multitask checkpoint에 복원 숫자 정확도만 추가할 때는 평가기를 한 번 학습한 뒤
multitask 평가와 그림을 다시 만든다. Multitask 모델 자체를 재학습할 필요는 없다.

```bash
CUBLAS_WORKSPACE_CONFIG=:4096:8 \
python main.py --model mnist --device cuda
