...
현재 저장된 결과 요약은 seed `[0, 1, 2]`로 수행한 기준 실험이다. Config에는
seed `0–9`가 설정되어 있으므로, 먼저 10-seed 실험으로 모델 간 차이의 방향과 분산을
다시 확인하고 결과 요약을 갱신한다.

그다음 단계에서는 현재 실험과 섞지 않고 다음 질문을 각각 별도 실험으로 다룬다.

- Class attention의 LeNet 대비 효과가 학습 seed에 따라 달라지는 원인 분석
- 낮은 attention AUPRC·IoU를 개선하기 위한 localization supervision 또는 regularization
- Reconstruction 보조 과제와 class attention의 독립 효과를 확인한 뒤 결합 가능성 검토
- 중앙 고정 배치에서 벗어난 위치·크기 변화에서도 같은 결론이 유지되는지 검증

이 항목들은 현재 결과의 일부가 아니며, 기존 controlled overlap 기준선과 분석 코드를
재사용하기 위한 후속 연구로 남겨둔다.
